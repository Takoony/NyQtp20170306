...
namespace Nyang
{
    public partial class Plat : UserControl
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            listInst.Sort();
            this.cbb_instrument1.Items.AddRange(listInst.ToArray());
            this.cbb_instrument2.Items.AddRange(listInst.ToArray());

            //时间周期
            this.cbb_interval.Items.AddRange(new[] { "Sec 1", "Min 1", "Min 3", "Min 5", "Min 10", "Min 15", "Min 30", "Hour 1", "Day 1", "Week 1", "Year 1" });
            ...
            _timer.Start();
            this.btn_add_strg.Click += btn_add_strg_Click;//添加策略
            this.btn_delete_strg.Click += btn_delete_strg_Click;
            this.btn_run_strg.Click += btn_run_strg_Click; //加载策略
            this.dgv_strategy_library.SelectionChanged += dgv_strategy_library_SelectionChanged;
            this.dgv_strategy_library.CellContentClick += dgv_strategy_library_CellContentClick;
            this.dgv_strategy_library.CellFormatting += dgv_strategy_library_CellFormatting;

            this.dgv_strategy_library.RowTemplate.DefaultCellStyle.SelectionBackColor = this.dgv_strategy_library.RowTemplate.DefaultCellStyle.BackColor;
            this.dgv_strategy_library.RowTemplate.DefaultCellStyle.SelectionForeColor = this.dgv_strategy_library.RowTemplate.DefaultCellStyle.ForeColor;
        }

        //格式化时间字段
        private void dgv_strategy_library_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            if (e.ColumnIndex < 0 || e.RowIndex < 0 || e.Value == null) return;

            if (this.dgv_strategy_library.Columns[e.ColumnIndex].HeaderText == "时间")
            {
                decimal val;
                DateTime dt;
        ...
        private void dgv_strategy_library_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            DataGridViewRow row = this.dgv_strategy_library.Rows[e.RowIndex];
            ...
            if (_dicStrategies.TryGetValue(name, out stra))
            {
                ...
                this.dgv_strategy_library.EndEdit();
                if (head == "Order")
                {
                    //勾选委托
                    if ((bool)this.dgv_strategy_library[e.ColumnIndex, e.RowIndex].Value)
        ...
        //加载策略
        private void btn_run_strg_Click(object sender, EventArgs e)
        {
            if (this.dgv_strategy_library.SelectedRows.Count == 0) return;
            ...
            //}
            DataGridViewRow row = this.dgv_strategy_library.SelectedRows[0];
            string name = (string)row.Cells["StraName"].Value;
            Strategy stra;
            if (!_dicStrategies.TryGetValue(name, out stra))
            {
           ...
           // this.propertyGridParams.Enabled = !this.propertyGridParams.Enabled;
            ...
            this.dgv_strategy_library.EndEdit();
            this.dgv_strategy_library.Refresh();

            if (data.IntervalType == EnumIntervalType.Sec || this.rbtn_tick.Checked) //秒周期或tick加载
            { }
            else
            {
                List<Bar> bars = null;
            ...
            dgv_strategy_library_SelectionChanged(null, null); //刷新委托列表
                                                                 //是否有结束日期:只测试
        ...
        //选择不同策略:显示策略成交记录
        private void dgv_strategy_library_SelectionChanged(object sender, EventArgs e)
        {
            _dtOrders.Rows.Clear();
            if (this.dgv_strategy_library.SelectedRows.Count == 0)
                return;
        ...
        void btn_add_strg_Click(object sender, EventArgs e)
        {
            if (this.cbb_strategy.SelectedIndex < 0) return;
            if (string.IsNullOrEmpty(this.cbb_instrument1.Text)) return;
            if (string.IsNullOrEmpty(this.cbb_instrument2.Text)) return;

            if (this.cbb_interval.SelectedIndex < 0) return;

            Strategy stra = (Strategy)Activator.CreateInstance((Type)this.cbb_strategy.SelectedItem);
            AddStra(stra, _dicStrategies.Count == 0 ? "1" : (_dicStrategies.Select(n => int.Parse(n.Key)).Max() + 1).ToString(), this.cbb_instrument1.Text, this.cbb_interval.Text, this.dtp_start.Value.Date, this.dtp_over.Checked ? this.dtp_over.Value.Date : DateTime.MaxValue);
        }
        ...
        private void btn_delete_strg_Click(object sender, EventArgs e)
        {
            if (this.dgv_strategy_library.SelectedRows.Count == 0) return;
            ...
            this.dgv_strategy_library.Rows.Remove(row);
        }
        ...
        void _timer_Tick(object sender, EventArgs e)
        {
            ...
            //更新时间与交易所状态
            foreach (DataGridViewRow row in this.dgv_strategy_library.Rows)
            {
                Strategy stra;
        ...
        private void SaveConfig()
        {
            ConfigFile cfg = new ConfigFile
            {
                StrategyFiles = new string[this.cbb_strategy_file.Items.Count],
                FloConfig = _t.FloConfig,
            };
            for (int i = 0; i < this.cbb_strategy_file.Items.Count; i++)
            {
                cfg.StrategyFiles[i] = (string)this.cbb_strategy_file.Items[i];
            }
        ...
        private int AddStra(Strategy stra, string pName, string pInstrument, string pInterval, DateTime pBegin, DateTime pEnd)
        {
            ...
            stra.Name = pName;
            int rid = this.dgv_strategy_library.Rows.Add(stra.Name, stra.GetType(), stra.GetParams(), pInstrument, pInterval, pBegin, pEnd);
            this.dgv_strategy_library.Rows[rid].Cells["Order"].Value = false;
            if (pEnd.Date == DateTime.MaxValue.Date)
                this.dgv_strategy_library.Rows[rid].Cells["EndDate"].Value = null;
        ...
        void LoadStrategy()
        {
            if (File.Exists("strategies.txt"))
            {
                ...
                for (int i = 1; i < lines.Length; ++i)
                {
                    ...
                    Type straType = null;
                    //类型是否存在
                    foreach (Type t in this.cbb_strategy.Items)
                    {
        ...
        //加载策略
        private void btn_load_strgs_file_click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (var t in ass.GetTypes())
                {
                    if (t.BaseType == typeof(Strategy))
                        this.cbb_strategy.Items.Add(t);
                }
                this.cbb_strategy_file.Items.Insert(0, file);
                this.cbb_strategy_file.SelectedIndex = 0;
            }
            catch (Exception err)
            {
        ...
        {
            this.cbb_strategy_file.Items.Clear();
            this.cbb_strategy_file.Text = string.Empty;
            this.cbb_strategy.Items.Clear();
        }
    }
